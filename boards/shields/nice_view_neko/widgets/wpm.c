...
#if IS_ENABLED(CONFIG_NICE_VIEW_NEKO_WPM_FIXED_RANGE)
    float max = CONFIG_NICE_VIEW_NEKO_WPM_FIXED_RANGE_MAX;
#else
    float max = 0;
    for (int i = 0; i < 10; i++) {
        if (state->wpm[i] > max) {
...
#if IS_ENABLED(CONFIG_NICE_VIEW_NEKO_WPM_FIXED_RANGE)
    int max = CONFIG_NICE_VIEW_NEKO_WPM_FIXED_RANGE_MAX;
    if (max == 0) {
        max = 100;
    }
